...
import type { AgentRules, Entry } from "./audit-types";
...
async function loadRules(sessionCwd?: string): Promise<AgentRules[]> {
	...
	if (sessionCwd) {
		for (const name of ["AGENTS.md", "CLAUDE.md"] as const) {
			for (const dir of [join(sessionCwd, ".pi"), sessionCwd]) {
...
export async function doAudit(entries: Entry[], sessionCwd?: string) {
	...
	for (const [rule, issues] of grouped) {
		...
		for (const issue of issues) {
			const scope =
				issue.fixScope !== "none"
					? ` [修复范围: ${issue.fixScope === "global" ? "全局 AGENTS.md" : "项目 CLAUDE.md"}]`
					: "";
			const fix = issue.fixSuggestion
				? `\n  💡 修复建议: ${issue.fixSuggestion}`
				: "";
			lines.push(
				`${icon[issue.severity]} ${issue.detail}${scope}`,
				`  证据: ${issue.evidence}${fix}`,
				"",
			);
		}
	}
