...
import { doChain, doEntries, doRaw, doSummary, doTimeline } from "../analyze";
...
const SAMPLE_ENTRIES = [
	{ type: "session", cwd: "/project", parentSession: "parent_abc123" },
	{
		...
		message: {
			role: "user",
			content: [{ type: "text", text: "Hello, please help" }],
			model: "gpt-4",
		},
	},
	{
	...
	{
		type: "message",
		timestamp: "2026-05-12T10:00:15.000Z",
...
function setupChainFiles() {
	...
	const childEntries = [
		// parentSession 包含 filepath 中的 test001，用于 doChain 的 parentSession 匹配
		{
			type: "session",
			cwd: "/project",
			parentSession: "20260512T100000_test001",
		},
		{
			type: "message",
			message: { role: "user", content: "child task", model: "gpt-4-mini" },
		},
	];
	fs.writeFileSync(
		childFile,
		childEntries.map((e) => JSON.stringify(e)).join("\n"),
		"utf-8",
	);
}
...
describe("doChain", () => {
	...
	it("显示父会话和子代理", async () => {
		const result = await doChain(
			SAMPLE_ENTRIES,
			"20260512T100000_test001.jsonl",
			tmpDir,
		);
		const text = result.content[0].text;
...
describe("doRaw", () => {
	...
	it("每个条目截断到 1000 字符", () => {
		const longEntry = [
			{ type: "message", message: { role: "user", content: "x".repeat(2000) } },
		];
