...
describe("parseSessionId", () => {
	...
	it("带路径的完整路径", () => {
		expect(
			parseSessionId(
				"/home/user/.pi/agent/sessions/20260512/20260512T170326_sessionXYZ.jsonl",
			),
		).toBe("sessionXYZ");
	});
...
describe("readJsonl", () => {
	...
	it("跳过空行", () => {
		const text = ['{"type":"session"}', "", "", '{"type":"message"}'].join(
			"\n",
		);
		const result = readJsonl(text);
		expect(result).toHaveLength(2);
...
describe("extractSummary", () => {
	...
	it("首条 user 消息被截断到 120 字符", () => {
		...
		const entries = [
			{
				type: "message",
				message: { role: "user", content: [{ type: "text", text: longMsg }] },
			},
		];
		const result = extractSummary(entries);
...
describe("getSessionInfoFromEntries", () => {
	it("提取会话元信息", () => {
		const entries = [
			...
			{
				type: "message",
				message: { role: "user", content: "hi", model: "gpt-4" },
			},
			{
				type: "message",
				message: {
		...
		const result = getSessionInfoFromEntries(
			entries,
			"20260512T170326_sess1.jsonl",
		);
...
describe("extractMatchContext", () => {
	const helloEntry = {
		...
		message: {
			role: "user",
			content: [{ type: "text", text: "Hello World from pi" }],
		},
	};
