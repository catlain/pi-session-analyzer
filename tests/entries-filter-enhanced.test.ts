...
const ENTRIES: Entry[] = [
	...
	} as Entry,
	{
		...
		message: {
			role: "toolResult",
			content: [{ type: "text", text: "project map result" }],
			toolName: "code_graph_project_map",
		},
	} as Entry,
	{
		type: "message",
		...
		message: {
			...
			content: [
				{
					type: "toolCall",
					name: "code_graph_module_overview",
	...
	} as Entry,
	{
		...
		message: {
			role: "toolResult",
			content: [{ type: "text", text: "module overview" }],
			toolName: "code_graph_module_overview",
		},
	} as Entry,
	{
		type: "message",
			...
			content: [
				{
					...
					arguments: { symbol_name: "doEntries", file_path: "entries.ts" },
				},
			],
		},
	} as Entry,
			...
			content: [
				{
					type: "toolCall",
					name: "bash",
					arguments: { command: "git commit -m 'refactor'" },
				},
			],
		},
	} as Entry,
...
function countShown(text: string): number {
	...
	if (m2) return parseInt(m2[2], 10) - parseInt(m2[1], 10) + 1;
	return -1;
}
