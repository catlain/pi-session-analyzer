		...
		return [
			{
				...
				evidence: `首次 edit 在条目 ${editIdx[0]}，之前无搜索`,
				fixScope: "none",
			},
		];
	}
...
export function checkSearchOnly(entries: Entry[]): AuditIssue[] {
	...
	for (const entry of entries) {
		...
		for (const part of content) {
			if (part.type !== "toolCall") continue;
			if (part.name === "web_search" || part.name === "gh_search_doc")
				searchCount++;
...
export function checkRepeatedErrors(entries: Entry[]): AuditIssue[] {
	...
	const errorMsg = new Map<
		string,
		{ tools: Set<string>; count: number; sample: string }
	>();
	for (const entry of entries) {
	...
	for (const [key, data] of errorMsg) {
		...
		if (toolCount >= 3) {
			...
			issues.push({
				...
				fixScope: "none",
				fixSuggestion:
					"这是 pi 工具框架 bug，不是项目代码问题。应重启会话或报告给用户",
			});
		} else if (toolCount >= 2) {
			// 跨 2 种工具 → warning
			issues.push({
		...
		return [
			{
				...
				evidence: `write=${writeCount}, edit=${editCount}`,
				fixScope: "none",
			},
		];
	}
	...
	return [
		{
			...
			fixSuggestion:
				"创建全局 AGENTS.md，包含文件格式、工具使用、编码规范等规则",
		},
	];
}
