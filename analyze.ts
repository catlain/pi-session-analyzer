...
import { assignBranch, buildEntryMap, findBranchPoints } from "./branches";
import {
	type Entry,
	extractSummary,
	extractText,
...
export { type DoEntriesOptions, doEntries, extractEntryText } from "./entries";

export function doTimeline(entries: Entry[]) {
	const msgEntries = entries.filter((e) => e.type === "message" && e.timestamp);
...
export async function doChain(
	...
	const parentSession = entries.find((e) => e.type === "session")
	...
	for (const fp of allFiles) {
		...
		if (sessionEntry?.parentSession) {
			...
			if (parentPath.includes(sessionId.slice(0, 14))) {
				...
				const model =
					childEntries.find((e) => e.type === "message" && e.message?.model)
						?.message?.model ?? "";
...
export function doBranches(entries: Entry[]) {
	...
	if (branchPoints.length === 0) {
		return {
			content: [
				{
					type: "text" as const,
					text: "该会话没有分支(/tree 操作产生的平行分支)。所有消息在一条线性链上。",
				},
			],
			details: {},
		};
	...
	const totalBranches = branchPoints.reduce(
		(s, bp) => s + bp.branches.length,
		0,
	);
	lines.push(
		`发现 ${branchPoints.length} 个分支点,共 ${totalBranches} 条分支\n`,
	);
